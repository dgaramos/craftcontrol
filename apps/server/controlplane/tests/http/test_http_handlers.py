...
from src.http.analytics import analytics_api
from src.http.core import core_api
from src.http.players import players_api
from src.http.server import server_api
from src.http.telemetry import telemetry_api
...
def test_update_config_returns_409_on_conflicting_operation(client, service: MagicMock) -> None:
    from src.operations import ConflictingOperationError
...
def test_telemetry_pack_status_returns_200(client, service: MagicMock) -> None:
    ...
    with patch("src.http.telemetry.telemetry_installer", return_value=fake_installer):
...
def test_telemetry_pack_status_error_returns_400(client, service: MagicMock) -> None:
    with patch("src.http.telemetry.telemetry_installer", side_effect=FileNotFoundError("no pack")):
        resp = client.get("/api/telemetry-pack")
    assert resp.status_code == 400
...
def _make_operations_app(manager: MagicMock, *, auth_mode: str = "disabled") -> Flask:
    from src.http.operations import operations_api as ops_bp
    app = Flask(__name__, template_folder="../../client/templates")
    app.extensions["manager_service"] = manager
    auth = make_auth_mock()
    wire_auth(app, auth, mode=auth_mode)
