...
from proxy.preflight.preflight import (
    Preflight,
    PreflightResult,
    SystemInfo,
    FakeRunner,
...
def test_preflight_never_writes_token_file():
    ...
    token_path = "/etc/craftcontrol/bedrock-proxy-token"
...
def test_preflight_installs_and_verifies_inactive_path_unit():
    ...
    assert ["systemctl", "enable", "--now", "craftcontrol-bedrock-proxy-env-acl.path"] in runner.calls
    assert ["systemctl", "start", "craftcontrol-bedrock-proxy-env-acl.service"] in runner.calls
...
def test_docker_membership_uses_configured_agent_not_invoking_process():
    ...
    installer = run_path(str(Path(__file__).parents[4] / "deploy/bedrock-proxy/bin/install-craftcontrol-bedrock-proxy-runtime"))
