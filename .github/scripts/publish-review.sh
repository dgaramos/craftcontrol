...
if [[ -n "${REVIEW_BODY:-}" ]] || [[ "$(jq length <<<"$inline_comments_json")" -gt 0 ]]; then
  publish_review=true
fi
...
while IFS= read -r thread_id; do
  ...
  while :; do
    graphql_args=(
      -f query='query($owner: String!, $name: String!, $number: Int!, $after: String) { repository(owner: $owner, name: $name) { pullRequest(number: $number) { reviewThreads(first: 100, after: $after) { nodes { id } pageInfo { hasNextPage endCursor } } } } }'
      -f owner="${GITHUB_REPOSITORY%%/*}"
      -f name="${GITHUB_REPOSITORY##*/}"
      -F number="$PR_NUMBER"
    )
    [[ -z "$cursor" ]] || graphql_args+=(-f after="$cursor")
    review_threads="$(gh api graphql "${graphql_args[@]}")"
    if jq -e --arg thread "$thread_id" '.data.repository.pullRequest.reviewThreads.nodes | any(.id == $thread)' <<<"$review_threads" >/dev/null; then
      thread_found=true
      break
    fi
    [[ "$(jq -r '.data.repository.pullRequest.reviewThreads.pageInfo.hasNextPage' <<<"$review_threads")" == true ]] || break
    cursor="$(jq -r '.data.repository.pullRequest.reviewThreads.pageInfo.endCursor' <<<"$review_threads")"
  done
  [[ "$thread_found" == true ]] || { echo "resolution target mismatch" >&2; exit 1; }
