...
def test_split_images_isolate_privileged_backend_from_frontend() -> None:
    ...
    backend_dockerfile = (ROOT / "apps" / "server" / "controlplane" / "Dockerfile").read_text()
    ...
    assert "apps/server/controlplane/Dockerfile" in backend
...
def test_frontend_proxy_preserves_same_origin_and_sse_streaming() -> None:
    nginx = (ROOT / "apps" / "client" / "deploy" / "nginx.conf").read_text()
...
def test_backend_image_does_not_bundle_frontend_application() -> None:
    backend = (ROOT / "apps" / "server" / "controlplane" / "Dockerfile").read_text()
