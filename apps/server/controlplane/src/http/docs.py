...
def contract_root(package_root: Path | None = None) -> Path:
    ...
    # src/ layout in source tree: .../apps/server/controlplane/src/
    if package_root.name == "src" and package_root.parent.name == "controlplane":
        return package_root.parents[3] / "packages" / "contracts"
    # src/ layout in Docker container: /app/src/
    if package_root.name == "src":
        return package_root.parent / "packages" / "contracts"
    # Legacy explicit path: .../apps/server/controlplane/
