...
return async function renderPlayerDetail(player, account, back = renderPlayersPanel) {
  ...
  try {
    ...
    renderMarkup(content, [
      ...
      `<span class="player-panel-title">${localized("Permissões", "Permissions", "Permisos")}</span>`,
      `</div>`,
      `<div class="player-panel-body">`,
      `<div class="hero-attr-control" title="${escapeHtml ? escapeHtml(operatorTooltip) : operatorTooltip}">`,
      `<span class="hero-attr-label">${localized("Operador", "Operator", "Operador")}</span>`,
      `<div class="player-operator-row">`,
      `<div id="detail-operator-control"></div>`,
      `<span id="detail-role-label"></span>`,
      `</div>`,
      `</div>`,
      `<div class="hero-attr-control">`,
      `<span class="hero-attr-label">${localized("Modo de jogo", "Game mode", "Modo de juego")}</span>`,
      `<select class="gamemode-select" id="detail-gamemode-select">`,
