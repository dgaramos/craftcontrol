...
class DockerContainerLogs:
    ...
    def started_at(self, container_name: str) -> str | None:
        ...
        try:
            result = self._subprocess_run(cmd, capture_output=True, text=True, timeout=10, check=False)
        except (subprocess.TimeoutExpired, OSError):
            return None
        if result.returncode != 0:
            return None
    ...
    def logs_since(self, container_name: str, since: str) -> str:
        ...
        try:
            result = self._subprocess_run(cmd, capture_output=True, text=True, timeout=30, check=False)
        except subprocess.TimeoutExpired as exc:
