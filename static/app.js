...
function playerBreakdownMarkup(entries, type, emptyLabel) {
  ...
  return `<ol class="player-data-ranking">${entries.map(([key, count], index) => `<li><b>${index + 1}</b>${type === "entity" ? gameTermMarkup(key) : `<span>${escapeHtml(type === "dimension" ? dimensionName(key) : blockName(key))}</span>`}<strong>${formatRankingValue(count, type === "dimension" ? "number" : "number")}</strong></li>`).join("")}</ol>`;
}
...
function playerDataMarkup(profile) {
  ...
  return `<section class="player-data-workspace"><header class="player-data-heading block-panel"><div><span class="eyebrow">PLAYER DATA</span><h3>${state.locale === "pt" ? "Dados individuais" : "Individual data"}</h3><p>${state.locale === "pt" ? `Tudo o que a telemetria conhece especificamente sobre ${escapeHtml(profile.name)}.` : `Everything telemetry knows specifically about ${escapeHtml(profile.name)}.`}</p></div><small>◆ ${t("authoritative")} · ${t("updated")} ${formatDate(profile.telemetry_updated_at)}</small></header><div class="telemetry-grid">${items.map(([label, value]) => `<span><b>${value || 0}</b>${t(label)}</span>`).join("")}</div><div class="player-data-grid"><section class="player-data-panel player-combat-data block-panel"><div class="player-data-panel-title"><span>${gameIcon("skeleton")}</span><div><small>COMBAT</small><h4>${state.locale === "pt" ? "Criaturas eliminadas" : "Creatures defeated"}</h4></div></div>${playerBreakdownMarkup(sortedTelemetryEntries(stats.killsByType), "entity", noKills)}</section><section class="player-data-panel block-panel"><div class="player-data-panel-title"><span>⛏</span><div><small>MINING</small><h4>${state.locale === "pt" ? "Blocos quebrados" : "Blocks broken"}</h4></div></div>${playerBreakdownMarkup(sortedTelemetryEntries(stats.brokenByType), "block", noBlocks)}</section><section class="player-data-panel block-panel"><div class="player-data-panel-title"><span>▦</span><div><small>BUILDING</small><h4>${state.locale === "pt" ? "Blocos colocados" : "Blocks placed"}</h4></div></div>${playerBreakdownMarkup(sortedTelemetryEntries(stats.placedByType), "block", noBlocks)}</section><section class="player-data-panel block-panel"><div class="player-data-panel-title"><span>◈</span><div><small>EXPLORATION</small><h4>${state.locale === "pt" ? "Dimensões visitadas" : "Dimensions visited"}</h4></div></div>${playerBreakdownMarkup(dimensions, "dimension", noDimensions)}</section></div></section>`;
}
