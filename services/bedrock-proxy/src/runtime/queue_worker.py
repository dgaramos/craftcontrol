...
---------------------
BEDROCK_PROXY_WORKERS      Number of worker threads.  Default: 1.
BEDROCK_PROXY_QUEUE_SIZE   Maximum items the queue holds before rejecting.  Default: 8.
"""
...
class OperationQueue:
    ...
    workers:
        ...
        ``BEDROCK_PROXY_WORKERS`` env var, falling back to ``WORKERS_DEFAULT``.
    queue_size:
        Maximum number of pending items.  Defaults to the
        ``BEDROCK_PROXY_QUEUE_SIZE`` env var, falling back to ``QUEUE_SIZE_DEFAULT``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if workers is None:
            workers = int(os.environ.get("BEDROCK_PROXY_WORKERS", WORKERS_DEFAULT))
        if queue_size is None:
            queue_size = int(os.environ.get("BEDROCK_PROXY_QUEUE_SIZE", QUEUE_SIZE_DEFAULT))

        if workers < 1:
            raise ValueError(f"BEDROCK_PROXY_WORKERS must be >= 1, got {workers}")
        if queue_size < 1:
            raise ValueError(f"BEDROCK_PROXY_QUEUE_SIZE must be >= 1, got {queue_size}")
