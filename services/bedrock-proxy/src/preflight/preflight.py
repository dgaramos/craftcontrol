...
Phases (executed in order; stops on first failure):
  ...
  5. Path-unit install: craftcontrol-bedrock-proxy-env-acl.{path,service}.
...
Security invariants enforced:
  ...
  - /etc/craftcontrol/bedrock-proxy-token never opened for writing.
...
_SERVICE = "craftcontrol-bedrock-proxy"
_TOKEN_PATH = "/etc/craftcontrol/bedrock-proxy-token"
...
_ENV_ACL_UNIT = "craftcontrol-bedrock-proxy-env-acl"
...
class Preflight:
    ...
    def _apply_acls(self) -> None:
        ...
        default_cmd = ["find", data, "-type", "d", "-exec", "setfacl", "-m", f"d:u:{user}:rwX", "{}", "+"]
        if not self._fs.has_acl(data, user, f"d:u:{user}:rwX"):
