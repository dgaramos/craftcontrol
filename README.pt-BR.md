...
Quando o CraftControl Host Agent está configurado (`HOST_AGENT_URL` definido), as operações de ciclo de vida do servidor — `PREPARATION` (escrita de configuração), `RESTART` (reinício do serviço Compose) e `HEALTH_WAIT` (sondagem de prontidão do Bedrock conforme o transporte: ping RakNet para `transport=raknet`, evidência do console para `transport=nethernet` no Bedrock 1.26.50+) — são delegadas a ele por meio de um canal HTTP autenticado. O agente cuida do acesso ao socket Docker para essas três etapas, evitando que o CraftControl Server as execute diretamente; o socket Docker continua montado no Server para conexão ao console Bedrock, streaming de logs e eventos Docker, que não fazem parte do contrato do Host Agent. Sem o Host Agent, o Server executa todas as operações de ciclo de vida diretamente.
