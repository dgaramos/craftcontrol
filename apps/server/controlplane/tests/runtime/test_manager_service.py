...
def test_rejects_exact_time_outside_one_day(manager_service: ManagerService) -> None:
    with pytest.raises(ValueError, match="fora do intervalo"):
        manager_service.time_action("set", {"value": 24001})


def test_queries_day_count(manager_service: ManagerService, fake_bedrock: FakeBedrock) -> None:
    result = manager_service.time_action("query", {"value": "day"})
    assert result["value"] == 34
    assert fake_bedrock.commands[-1] == ["time", "query", "day"]


def test_operator_access_can_be_enabled_and_disabled(manager_service: ManagerService, fake_bedrock: FakeBedrock) -> None:
    manager_service.set_player_operator("VonCrush", True)
    assert fake_bedrock.commands[-1] == ["op", "VonCrush"]
    assert manager_service.players()[0]["name"] == "VonCrush"
    assert manager_service.players()[0]["operator"]
    manager_service.set_player_operator("VonCrush", False)
    assert fake_bedrock.commands[-1] == ["deop", "VonCrush"]


def test_player_disconnect_preserves_profile_and_closes_session(manager_service: ManagerService) -> None:
    manager_service.player_event("Nicole", True, "123")
    manager_service.player_event("Nicole", False, "123")
    profile = manager_service.players()[0]
    assert profile["name"] == "Nicole"
    assert not profile["online"]
    assert profile["sessions_count"] == 1


def test_death_counter_is_deduplicated(manager_service: ManagerService) -> None:
    manager_service.player_event("Nicole", True, "123")
    raw = "[INFO] Nicole was slain by Zombie"
    assert manager_service.player_death_event("Nicole", "was slain by Zombie", raw)
    assert not manager_service.player_death_event("Nicole", "was slain by Zombie", raw)
    assert manager_service.players()[0]["deaths_count"] == 1


def test_snapshot_response_is_ingested_without_log_stream(manager_service: ManagerService) -> None:
    manager_service.request_telemetry_snapshot_async("test")
    deadline = time.time() + 2
    while time.time() < deadline and manager_service.state().get("telemetry", {}).get("status") != "healthy":
        time.sleep(0.01)
    telemetry = manager_service.state()["telemetry"]
    assert telemetry["status"] == "healthy"
    assert telemetry["sequence"] == "12"


def test_manual_snapshot_is_ingested_synchronously(manager_service: ManagerService) -> None:
    assert manager_service.request_telemetry_snapshot("manual") == 2
    assert manager_service.state()["telemetry"]["last_topic"] == "snapshot.finished"


def test_diagnostics_summarize_telemetry_and_broker_counters(manager_service: ManagerService) -> None:
    manager_service.telemetry_event(telemetry_envelope("blocks.changed", sequence=10, timestamp=1))
    manager_service.telemetry_event(telemetry_envelope("blocks.changed", sequence=10, timestamp=2))
    manager_service.telemetry_event(telemetry_envelope("blocks.changed", sequence=9, timestamp=3))

    diagnostics = manager_service.diagnostics()
...
def test_telemetry_event_rejects_boolean_sequence(manager_service: ManagerService) -> None:

    with pytest.raises(ValueError, match="sequence must be an integer"):
        manager_service.telemetry_event({**telemetry_envelope("blocks.changed"), "sequence": True})
...
def test_diagnostics_count_repository_rejection_by_snapshot_topic(manager_service: ManagerService) -> None:
    envelope = telemetry_envelope("snapshot.started", player=None)

    manager_service.telemetry_event(envelope)
    manager_service.telemetry_event(envelope)

    assert manager_service.diagnostics()["telemetry"]["by_topic"]["snapshot.started"]["rejected"] == 1


def test_empty_snapshot_response_is_degraded_instead_of_stuck_syncing(manager_service: ManagerService, fake_bedrock: FakeBedrock) -> None:
    fake_bedrock.telemetry_output = ""
    assert manager_service.request_telemetry_snapshot("test-empty") == 0
    telemetry = manager_service.state()["telemetry"]
    assert telemetry["status"] == "degraded"
    assert telemetry["last_error"] == "snapshot returned no envelopes"


def test_snapshot_lines_without_telemetry_prefix_are_skipped(manager_service: ManagerService, fake_bedrock: FakeBedrock) -> None:
    fake_bedrock.telemetry_output = "INFO: some unrelated log line\nDEBUG: another line"
    assert manager_service.request_telemetry_snapshot("test-no-prefix") == 0
    assert manager_service.state()["telemetry"]["status"] == "degraded"


def test_sequence_gap_degrades_and_requests_reconciliation(manager_service: ManagerService) -> None:
    requested: list[str] = []
    manager_service.request_telemetry_snapshot_async = requested.append  # type: ignore[method-assign]
    manager_service.telemetry_event(telemetry_envelope("blocks.changed", sequence=10, timestamp=1))
    manager_service.telemetry_event(telemetry_envelope("blocks.changed", sequence=13, timestamp=2))
    telemetry = manager_service.state()["telemetry"]
    assert telemetry["status"] == "degraded"
    ...
    assert manager_service.diagnostics()["telemetry"]["sequence"]["lost"] == 2
    assert manager_service.diagnostics()["telemetry"]["by_topic"]["blocks.changed"]["gaps"] == 1


def test_snapshot_repairs_degraded_state_and_stale_delta_is_rejected(manager_service: ManagerService) -> None:
    manager_service.request_telemetry_snapshot_async = lambda reason: None  # type: ignore[method-assign]
    manager_service.telemetry_event(telemetry_envelope("blocks.changed", sequence=10, timestamp=1))
    manager_service.telemetry_event(telemetry_envelope("blocks.changed", sequence=12, timestamp=2))
    manager_service.telemetry_event(telemetry_envelope("player.joined", sequence=11, timestamp=3))
    assert manager_service.state()["telemetry"]["sequence"] == "12"
    manager_service.telemetry_event(telemetry_envelope("snapshot.started", sequence=12, timestamp=4, player=None, data={"players": 0}))
    manager_service.telemetry_event(telemetry_envelope("snapshot.finished", sequence=12, timestamp=5, player=None))
    telemetry = manager_service.state()["telemetry"]
    assert telemetry["status"] == "healthy"
...
def test_pack_sequence_reset_requests_snapshot(manager_service: ManagerService) -> None:
    requested: list[str] = []
    manager_service.request_telemetry_snapshot_async = requested.append  # type: ignore[method-assign]
    manager_service.telemetry_event(telemetry_envelope("blocks.changed", sequence=20, timestamp=1))
    manager_service.telemetry_event(telemetry_envelope("telemetry.started", sequence=1, timestamp=2, player=None, data={"version": "0.2.0"}))
    telemetry = manager_service.state()["telemetry"]
    assert telemetry["status"] == "syncing"
    assert telemetry["reset_count"] == "1"
    assert requested == ["pack-started"]
    assert manager_service.diagnostics()["telemetry"]["by_topic"]["telemetry.started"]["resets"] == 1
    assert manager_service.diagnostics()["telemetry"]["sequence"]["resets"] == 1


def test_sequence_losses_are_global_when_a_different_topic_detects_the_gap(manager_service: ManagerService) -> None:
    manager_service.request_telemetry_snapshot_async = lambda reason: None  # type: ignore[method-assign]
    manager_service.telemetry_event(telemetry_envelope("player.joined", sequence=10, timestamp=1))
    manager_service.telemetry_event(telemetry_envelope("blocks.changed", sequence=13, timestamp=2))

    telemetry = manager_service.diagnostics()["telemetry"]
...
def test_blocked_pack_storage_stays_degraded_after_snapshot(manager_service: ManagerService) -> None:
    manager_service.request_telemetry_snapshot_async = lambda reason: None  # type: ignore[method-assign]
    storage = {"status": "blocked", "storageVersion": 1, "persistenceBlocked": True, "error": "invalid persisted state"}
    manager_service.telemetry_event(telemetry_envelope("telemetry.started", sequence=1, timestamp=1, player=None, data={"storage": storage}))
    manager_service.telemetry_event(telemetry_envelope("snapshot.started", sequence=1, timestamp=2, player=None, data={"players": 0, "storage": storage}))
    manager_service.telemetry_event(telemetry_envelope("snapshot.finished", sequence=1, timestamp=3, player=None))
    telemetry = manager_service.state()["telemetry"]
    assert telemetry["status"] == "degraded"
...
def test_pack_storage_migration_is_reported_separately_from_protocol(manager_service: ManagerService) -> None:
    manager_service.request_telemetry_snapshot_async = lambda reason: None  # type: ignore[method-assign]
    storage = {"status": "migrated", "storageVersion": 1, "migratedFrom": 0, "persistenceBlocked": False}
    manager_service.telemetry_event(telemetry_envelope("telemetry.started", sequence=8, timestamp=1, player=None, data={"storage": storage}))
    telemetry = manager_service.state()["telemetry"]
...
def test_pack_capabilities_are_persisted_without_degrading_supported_metrics(manager_service: ManagerService) -> None:
    manager_service.request_telemetry_snapshot_async = lambda reason: None  # type: ignore[method-assign]
    capabilities = {
        "blocksBroken": {"supported": True},
        "dimensionChanges": {"supported": False},
    }
    manager_service.telemetry_event(telemetry_envelope("telemetry.started", sequence=4, timestamp=1, player=None, data={"capabilities": capabilities}))
    telemetry = manager_service.state()["telemetry"]
...
def test_refresh_stores_settings_and_server_info(manager_service: ManagerService, fake_bedrock: FakeBedrock) -> None:
    gamerules = {"keepInventory": "false"}
    fake_bedrock.query_state_result = (gamerules, ["Alice"], 1, 10, {"Alice": "111"})
    manager_service.refresh("test")
    state = manager_service.state()
    assert state["online"] == 1
    assert state["max_players"] == 10
    assert state["gamerules"]["keepInventory"] == "false"


def test_refresh_max_players_does_not_read_deployment_env(tmp_path: Path, manager_service: ManagerService, fake_bedrock: FakeBedrock) -> None:
    (tmp_path / ".env").write_text("MAX_PLAYERS=20\n")
    fake_bedrock.query_state_result = ({}, [], 0, 0, {})
    manager_service.refresh("test")
    assert manager_service.state()["max_players"] == 0


def test_refresh_error_publishes_failed_event_and_reraises(manager_service: ManagerService, fake_bedrock: FakeBedrock) -> None:
    fake_bedrock.query_state_error = RuntimeError("docker gone")
    events: list[str] = []
    original_publish = manager_service.broker.publish

    def capture(topic: str, *args, **kwargs):
        events.append(topic)
        return original_publish(topic, *args, **kwargs)

    manager_service.broker.publish = capture  # type: ignore[method-assign]
...
def test_concurrent_refresh_is_skipped(manager_service: ManagerService, fake_bedrock: FakeBedrock) -> None:
    ...
    fake_bedrock.query_state = slow_query  # type: ignore[method-assign]
    t = threading.Thread(target=manager_service.refresh, args=("bg",))
    t.start()
...
def test_public_state_hides_known_players_and_bootstrap(manager_service: ManagerService, fake_bedrock: FakeBedrock) -> None:
    fake_bedrock.query_state_result = ({}, ["Alice"], 1, 5, {"Alice": "111"})
    manager_service.refresh("test")
    pub = manager_service.public_state()
    assert "known_players" not in pub
    assert "bootstrap" not in pub
    assert "online" in pub


def test_refreshing_flag_is_false_after_refresh(manager_service: ManagerService) -> None:
    manager_service.refresh("test")
    assert not manager_service.refreshing
...
def test_initialize_without_runtime_starts_refresh(manager_service: ManagerService) -> None:
    refreshed = threading.Event()
    original = manager_service._reconciliation.refresh

    def refresh_and_signal(reason: str = "manual") -> None:
        original(reason)
        refreshed.set()

    manager_service._reconciliation.refresh = refresh_and_signal  # type: ignore[method-assign]
    manager_service.initialize()
    assert refreshed.wait(timeout=10), "refresh did not run"


def test_initialize_starts_runtime_when_attached(manager_service: ManagerService) -> None:
    refreshed = threading.Event()
    original = manager_service._reconciliation.refresh
    ...
    manager_service._reconciliation.refresh = refresh_and_signal  # type: ignore[method-assign]
    runtime = FakeRuntime()
    manager_service.attach_runtime(runtime)  # type: ignore[arg-type]
    manager_service.initialize()
    assert refreshed.wait(timeout=10), "refresh did not run"
    assert runtime.started


def test_attach_runtime_twice_raises(manager_service: ManagerService) -> None:
    runtime = FakeRuntime()
    manager_service.attach_runtime(runtime)  # type: ignore[arg-type]
    with pytest.raises(RuntimeError):
        manager_service.attach_runtime(runtime)  # type: ignore[arg-type]
...
def test_save_known_setting_persists_to_bedrock_properties_and_returns_keys(tmp_path: Path, manager_service: ManagerService) -> None:
    changed, operation_id = manager_service.save_settings({"SERVER_NAME": "TestServer"})
...
def test_save_settings_rejects_non_dict(manager_service: ManagerService) -> None:
    with pytest.raises(TypeError):
        manager_service.save_settings("bad")  # type: ignore[arg-type]


def test_save_settings_rejects_unknown_keys(manager_service: ManagerService) -> None:
    with pytest.raises(ValueError):
        manager_service.save_settings({"__unknown__": "value"})
...
def test_set_known_gamerule_sends_command(manager_service: ManagerService, fake_bedrock: FakeBedrock) -> None:
    from src.core.schema import GAMERULES
    rule = next(iter(GAMERULES))
    schema = GAMERULES[rule]
    value = schema.get("default", "false")
    manager_service.set_gamerule(rule, value)
    assert ["gamerule", rule, str(value)] in fake_bedrock.commands


def test_set_unknown_gamerule_raises_key_error(manager_service: ManagerService) -> None:
    with pytest.raises(KeyError):
        manager_service.set_gamerule("__not_a_rule__", "true")
...
def test_run_invalid_world_action_raises_key_error(manager_service: ManagerService) -> None:
    with pytest.raises(KeyError):
        manager_service.run_world_action("__nope__")
...
def test_add_time_out_of_range_raises(manager_service: ManagerService) -> None:
    with pytest.raises(ValueError, match="fora do intervalo"):
        manager_service.time_action("add", {"value": 0})
...
def test_weather_duration_out_of_range_raises(manager_service: ManagerService) -> None:
    with pytest.raises(ValueError, match="fora do intervalo"):
        manager_service.time_action("weather", {"value": "rain", "duration": "0"})
...
def test_weather_query_returns_unknown_for_unrecognised_output(manager_service: ManagerService, fake_bedrock: FakeBedrock) -> None:
    fake_bedrock.send_and_read = lambda parts: "something else"  # type: ignore[method-assign]
    result = manager_service.time_action("weather-query", {})
    assert result["value"] == "unknown"


def test_invalid_action_raises_key_error(manager_service: ManagerService) -> None:
    with pytest.raises(KeyError):
        manager_service.time_action("__bad__", {})


def test_invalid_preset_raises_key_error(manager_service: ManagerService) -> None:
    with pytest.raises(KeyError):
        manager_service.time_action("preset", {"value": "__bad__"})
...
def test_second_request_within_cooldown_is_coalesced(manager_service: ManagerService) -> None:
    first_started = threading.Event()
    call_count = 0
    original_snapshot = manager_service._reconciliation.request_telemetry_snapshot
    ...
    manager_service._reconciliation.request_telemetry_snapshot = counted_snapshot  # type: ignore[method-assign]
    ...
    manager_service.request_telemetry_snapshot_async("first")
    manager_service.request_telemetry_snapshot_async("second")
...
def test_players_returns_known_player_fields(manager_service: ManagerService) -> None:
    manager_service.player_event("VonCrush", True, "xuid-1")
    players = manager_service.players()
    assert len(players) == 1
    assert players[0]["name"] == "VonCrush"
    assert "online" in players[0]


def test_player_profile_returns_none_for_unknown(manager_service: ManagerService) -> None:
    assert manager_service.player_profile("__nobody__") is None


def test_player_profile_returns_profile_for_known_player(manager_service: ManagerService) -> None:
    manager_service.player_event("VonCrush", True, "xuid-1")
    player_id = manager_service.players()[0]["id"]
    profile = manager_service.player_profile(player_id)
    assert profile is not None
    assert profile["name"] == "VonCrush"


def test_close_online_sessions_returns_list(manager_service: ManagerService) -> None:
    manager_service.player_event("Bob", True, "999")
    closed = manager_service.close_online_sessions("server-stop")
...
def test_missing_player_service_raises_type_error(tmp_path: Path) -> None:
    ...
    from src.server.files import ServerFiles
    from src.runtime import ReconciliationService
...
def test_save_settings_raises_conflicting_operation_on_concurrent_request(tmp_path: Path) -> None:
    from src.operations.service import ConflictingOperationError
    from src.operations.lifecycle import ServerOperation
    from src.operations.repository import SQLiteOperationRepository
    service = _make_service_with_operation_service(tmp_path)
...
def test_batch_diagnostics_start_empty(manager_service: ManagerService) -> None:
    blocks = manager_service.diagnostics()["telemetry"]["blocks"]
    assert blocks == {"count": 0, "total_blocks_declared": 0, "max_blocks_declared": 0}


def test_batch_diagnostics_single_batch(manager_service: ManagerService) -> None:
    manager_service.telemetry_event(_blocks_changed(10, broken_total=3, placed_total=2))
    blocks = manager_service.diagnostics()["telemetry"]["blocks"]
...
def test_batch_diagnostics_multiple_batches(manager_service: ManagerService) -> None:
    manager_service.telemetry_event(_blocks_changed(10, broken_total=2, placed_total=1))   # 3
    manager_service.telemetry_event(_blocks_changed(11, broken_total=5, placed_total=2))   # 7
    manager_service.telemetry_event(_blocks_changed(12, broken_total=1, placed_total=1))   # 2
    blocks = manager_service.diagnostics()["telemetry"]["blocks"]
...
def test_batch_diagnostics_missing_total(manager_service: ManagerService) -> None:
    envelope = telemetry_envelope("blocks.changed", sequence=10, timestamp=10)
    manager_service.telemetry_event(envelope)
    blocks = manager_service.diagnostics()["telemetry"]["blocks"]
    assert blocks["count"] == 1
...
def test_batch_rejected_does_not_count(manager_service: ManagerService) -> None:
    manager_service.telemetry_event(_blocks_changed(10, broken_total=3, placed_total=2))
...
def test_snapshot_diagnostics_empty(manager_service: ManagerService) -> None:
    snapshots = manager_service.diagnostics()["telemetry"]["snapshots"]
...
def test_snapshot_diagnostics_complete(manager_service: ManagerService) -> None:
    manager_service.telemetry_event(_snapshot_started(1))
    manager_service.telemetry_event(_snapshot_finished(2))
    snapshots = manager_service.diagnostics()["telemetry"]["snapshots"]
...
def test_snapshot_diagnostics_multiple(manager_service: ManagerService) -> None:
    manager_service.telemetry_event(_snapshot_started(1))
    manager_service.telemetry_event(_snapshot_finished(2))
    manager_service.telemetry_event(_snapshot_started(3))
    manager_service.telemetry_event(_snapshot_finished(4))
    snapshots = manager_service.diagnostics()["telemetry"]["snapshots"]
    assert snapshots["count"] == 2
    assert snapshots["duration_ms_total"] >= snapshots["duration_ms_max"]


def test_snapshot_last_player_count(manager_service: ManagerService) -> None:
    manager_service.telemetry_event(_snapshot_started(1))
    manager_service.telemetry_event(_snapshot_finished(2, players=["p1", "p2"]))
    assert manager_service.diagnostics()["telemetry"]["snapshots"]["last_player_count"] == 2


def test_snapshot_no_players_field(manager_service: ManagerService) -> None:
    manager_service.telemetry_event(_snapshot_started(1))
    manager_service.telemetry_event(_snapshot_finished(2))
    assert manager_service.diagnostics()["telemetry"]["snapshots"]["last_player_count"] is None


def test_snapshot_started_without_finished(manager_service: ManagerService) -> None:
    manager_service.telemetry_event(_snapshot_started(1))
    assert manager_service.diagnostics()["telemetry"]["snapshots"]["count"] == 0


def test_mixed_ingest_no_cross_contamination(manager_service: ManagerService) -> None:
    manager_service.telemetry_event(_blocks_changed(10, broken_total=5, placed_total=0))
    manager_service.telemetry_event(_snapshot_started(11))
    manager_service.telemetry_event(_snapshot_finished(12, players=["VonCrush"]))
    blocks = manager_service.diagnostics()["telemetry"]["blocks"]
    snapshots = manager_service.diagnostics()["telemetry"]["snapshots"]
    assert blocks["count"] == 1
    assert blocks["total_blocks_declared"] == 5
