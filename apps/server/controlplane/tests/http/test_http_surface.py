...
from src.auth.http import auth_api
...
def test_flask_compatibility_serves_frontend_from_the_application_boundary(tmp_path: Path) -> None:
    ...
    assert b'import { startApplication } from "./js/composition.js?v=45"' in script_data
    assert composition_status == 200
    assert b'import { state } from "./core/state.js?v=8"' in composition_data
