...
from src.runtime import EventRuntime
...
def test_telemetry_line_dispatched(log_runtime) -> None:
    runtime, _, service = log_runtime
    from src.telemetry.telemetry import PREFIX
    service.telemetry_event = MagicMock()
    with patch("src.runtime.supervisor.parse_telemetry_line") as mock_parse:
...
def test_telemetry_json_error_publishes_rejected(log_runtime) -> None:
    runtime, broker, service = log_runtime
    from src.telemetry.telemetry import PREFIX
    with patch("src.runtime.supervisor.parse_telemetry_line", side_effect=json.JSONDecodeError("bad", "", 0)):
...
def test_telemetry_none_return_skips_service(log_runtime) -> None:
    runtime, broker, service = log_runtime
    from src.telemetry.telemetry import PREFIX
    with patch("src.runtime.supervisor.parse_telemetry_line", return_value=None):
