...
function combatBreakdown(title, entries, icon) {
  ...
  return `<section class="combat-breakdown block-panel"><div class="ranking-section-title"><span class="eyebrow">${uiIcon(icon)} ${t("lifetime")}</span><h3>${title}</h3></div>${entries.length ? `<ol>${entries.map((entry, index) => `<li><b>${index + 1}</b>${gameTermMarkup(entry.key, kind)}<strong>${formatRankingValue(entry.count, "number")}</strong></li>`).join("")}</ol>` : `<div class="combat-zero"><span>${uiIcon(icon)}</span><p>${t("noCombatEvidence")}</p></div>`}</section>`;
}

function combatPlayerRanking(entries, title) {
  return `<section class="block-player-ranking block-panel"><div class="ranking-section-title"><span class="eyebrow">${t("lifetime")}</span><h3>${title}</h3></div>${entries.length ? `<ol>${entries.map((entry, index) => `<li><b>${index + 1}</b><button data-combat-player="${escapeHtml(entry.player.id)}" type="button">${escapeHtml(entry.player.name)}</button><strong>${formatRankingValue(entry.value, "number")}</strong></li>`).join("")}</ol>` : `<div class="combat-zero"><span>${uiIcon("combat")}</span><p>${t("noCombatEvidence")}</p></div>`}</section>`;
}

return async function renderCombatPanel() {
  const analytics = state.analytics;
  // all API data paths use escapeHtml or formatRankingValue.
  renderMarkup(content, `<div class="combat-screen">${analyticsViewSwitch("combat")}<header class="combat-hero block-panel"><div><span class="eyebrow">${t("combatLogKicker")}</span><h2>${t("combatTitle")}</h2><p>${t("combatHelp")}</p></div><button id="combat-refresh" class="secondary" type="button">${uiIcon("refresh")} ${t("refreshData")}</button></header><div id="combat-content"><div class="analytics-loading">${t("checking")}</div></div></div>`);  bindAnalyticsViewSwitch();
  const load = async () => {
