...
import { STATE_BACKUP_KEY, STATE_KEY } from "../behavior_pack/scripts/model.js";
...
assert.equal(migrated.storageVersion, 1);
assert.equal(migrated.sequence, 10);
assert.equal(migrated.players.voncrush.deaths, 4);
assert.equal(migrated.players.voncrush.blocksBroken, 12);
assert.ok(output.some((line) => line.includes("[BEDROCK_TELEMETRY_MIGRATION] storage 0 -> 1 complete")));
...
assert.equal(started.data.storage.storageVersion, 1);
