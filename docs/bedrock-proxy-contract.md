...
|----------|---------|-------------|
| `BEDROCK_PROXY_URL` | `http://host-gateway:7890` | Base URL the backend uses to reach the agent. |
...
containers. The agent source and tests live under `services/bedrock-proxy/`. The systemd unit file is available under `deploy/bedrock-proxy/systemd/`.

| Property | Value |
|----------|-------|
| Service name | `craftcontrol-bedrock-proxy` |
...
| Secret file | `/etc/craftcontrol/bedrock-proxy-token`, mode `0600`, owned by `craftcontrol-agent` |
...
`apps/server/controlplane/server/bedrock_proxy.py`) calls this agent over HTTP and
...
Configuration selects exactly one implementation at startup: when `BEDROCK_PROXY_URL`
...
The direct Compose adapter (`DockerOperations`) and the bedrock-proxy adapter
